...
@app.on_message(filters.text & ~filters.command(["file", "delete"]))
...
# Step 4: Handle /delete command to clear the file
@app.on_message(filters.command("delete"))
def delete_file(client, message):
    try:
        with open(MOVIE_FILE, "w") as file:
            file.write("")  # Clear the file contents
        message.reply("All entries in movie.txt have been deleted.")
    except Exception as e:
        message.reply(f"An error occurred while deleting the file: {str(e)}")
